...
from scipy.stats import randint, binom, norm, geom, poisson, logser, stats

import distributionClass as dist
import pandas as pd


def kolmogorov_criterion(sample, p, countExp):
    n = len(sample)
    print(stats.kstest(sample, 'binom', [countExp, p]))

    x = np.array([i for i in range(countExp)])
    cdf = binom.cdf(x, countExp, p)
    d_n = max([abs(cdf[i] - len(list(filter(lambda xi: xi <= i, sample))) / n) for i in range(countExp)])
    s_k = d_n * np.sqrt(n)
    print(s_k)
